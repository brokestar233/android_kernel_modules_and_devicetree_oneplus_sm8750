			      ...
			      struct qdf_mac_addr *mac_addr_tx_allowed)
{
	...
	case QDF_MONITOR_MODE:
		/* For monitor mode, allow all frames to be transmitted
		 * as this is used for frame injection. Copy the source
		 * MAC address from the frame itself.
		 */
		qdf_copy_macaddr(mac_addr_tx_allowed,
				 (struct qdf_mac_addr *)
				 (qdf_nbuf_data(nbuf) + QDF_NBUF_SRC_MAC_OFFSET));
		break;
#endif
	default:
		break;
...
dp_start_xmit(struct wlan_dp_link *dp_link, qdf_nbuf_t nbuf)
{
	 ...
	 * as Monitor mode is used for frame injection
	 * and does not require association with a specific peer or protocol processing.
	 */
	...
	/*
	 * For Monitor mode, skip peer state checks,
	 * as Monitor mode is used for frame injection
